...
int main()
{
    ...
    std::sort(v.begin(), v.end(),
              [center](const auto x, const auto y) -> bool { return abs(center - x) < abs(center - y); }); // 1
    print(v, "center 50:");
    ...
    auto positiveEvenFirst = [](const auto x, const auto y) -> bool {
        if (x * y <= 0)
        {
            return x > y;
        }
    ...
    srand(2022);
    int a = 0, b = 40;
    auto generator = [&a, &b]() -> int { return (std::rand() % (b - a + 1)) + a; }; /// 3
    ...
    int start = 5, step = 2;
    auto arithmeticGenerator = [start2 = start, &step]() mutable {
        int tmp = start2;
        start2 += step;
        return tmp;
    }; // [ 4 ]
    ...
    auto l1_norm = [](auto container) {
        auto addAbsoluteValue = [](auto start, double x) { return start + std::abs(x); };
        return std::accumulate(container.begin(), container.end(), 0.0, addAbsoluteValue);
    }; // [ 5 ]
    ...
    auto polynomial = [](double *a, int n) {
        return [a, n](double point) -> double {
            double value = 0;

            for (std::size_t i = 0; i < n + 1; i++)
            {
                value += a[i] * std::pow(point, i);
            }
